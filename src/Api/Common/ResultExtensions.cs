...
namespace KartOfferService.Api.Common;
...
/// `Kart.Shared.ErrorHandling`'s exception-mapping path produces for thrown exceptions
/// (`KartProblemDetailsFactory`) - one consistent error shape platform-wide, regardless of which
/// path in a handler produced the rejection.
/// </summary>
public static class ResultExtensions
{
    ...
    public static ActionResult MapFailure(this ControllerBase controller, Error error)
    {
        var statusCode = error.Code switch
        {
            ...
            "conflict" or ErrorCodes.CouponExpired => StatusCodes.Status409Conflict,
            ErrorCodes.StaleVersion => StatusCodes.Status412PreconditionFailed,
